...
public class PedidoController extends DaoImplementacao<Pedido> implements
	...
	@RequestMapping(value = "graficopedidos", method = RequestMethod.GET)
	public @ResponseBody String graficoCompras() {

		String sql = "select c.nome, SUM(cast( cast(replace(p.valortotal,'R$','') as text) as float)) as Total"
				+ " from pedido p"
				+ " inner join  cliente c on c.id = p.cliente_id"
				+ " group by c.nome order by Total";
		...
		retorno[cont] = "[\"" + "Cliente" +  "\"," + "\"" + "Gastos " + "\"]";
		cont ++;
		
		for (Object[] object : lista) {
			//Cliente posição 1  gastos na posição 0
			retorno[cont] = "[\"" + object[0] +  "\"," + "\"" + object[1] + "\"]";
			cont ++;
		}
	...
	public @ResponseBody String grafico() {
		...
		retorno[cont] = "[\"" + "Livros" +  "\"," + "\"" + "Quantidade " + "\"]";
		cont ++;
		
		for (Object[] object : lista) {
			//livro posição 1  quantidade na posição 0
